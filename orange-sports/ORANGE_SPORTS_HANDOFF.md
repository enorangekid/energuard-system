...
> 제품 분리 결정(2026-09-03): 에너가드 통합관리자 페이지에서는 스포츠 위젯을 더 이상 제공하지 않는다. `orange-sports/`의 Orange Sports를 독립 제품으로 유지하고, 향후 별도 웹서비스를 본체로 만든다. 통합 페이지의 위젯 진입 버튼·패널 HTML·`js/widget-sports.js` 로딩은 제거했다.
