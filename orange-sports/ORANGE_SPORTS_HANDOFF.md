# Orange Sports 작업 인수인계

최종 갱신: 2026-09-03
대상 저장소: `energuard-system` / `main`

> 제품 분리 결정(2026-09-03): 에너가드 통합관리자 페이지에서는 스포츠 위젯을 더 이상 제공하지 않는다. `orange-sports/`의 Orange Sports를 독립 제품으로 유지하고, 로컬 웹 프로토타입을 확장한다. 통합 페이지의 위젯 진입 버튼·패널 HTML·`js/widget-sports.js` 로딩은 제거했다.

> 운영 방향 변경(2026-09-03): 공개·상업 스포츠 니치 사이트 계획은 보류했다. 당분간 개인 사용 또는 소수 사용자에게 직접 배포하는 로컬 앱·로컬 웹 프로토타입으로만 유지한다. `orange-sports-web/`의 뉴스 수집과 외부 데이터 연동은 공개 서비스용 라이선스를 확보한 구성이 아니므로 그대로 공개 호스팅하지 않는다.
...
> 2026-09-03 알림 업데이트: NBA 쿼터·연장 종료, MLB 이닝 초·말 종료, EPL/UCL 전반전·경기 종료에 현재 점수를 포함한 카드 알림을 추가했다. 버전 2.5.0 → 2.5.1.

> 2026-09-03 필터 수정: 진행 중 경기 알림을 하나 이상 직접 선택한 경우 선택한 경기만 감시하도록 우선순위를 바로잡았다. 버전 2.5.1 → 2.5.2.

> 2026-09-03 아이콘 업데이트: 사용자 제작 `icon.png`을 앱 창, 알림 창, 트레이, 실행 파일 및 설치 프로그램 아이콘으로 적용했다. 버전 2.5.2 → 2.5.3.

> 2026-09-03 알림 배치 업데이트: 카드 크기는 유지하면서 화면 우측·하단 실질 여백을 약 3px로 줄이고 알림 간 간격을 정리했다. 버전 2.5.3 → 2.5.4.

> 2026-09-03 알림 외형 업데이트: 교체된 224×224 `icon.png`을 반영하고 알림 카드 라운드를 17px에서 11px로 줄였다. 버전 2.5.4 → 2.5.5.

> 2026-09-03 다종목 동시 알림 수정: 응원 팀과 직접 찜한 경기를 합집합으로 감시하고, 현재 보고 있는 종목과 무관하게 NBA·MLB·EPL·UCL을 동시에 감시하도록 수정했다. 버전 2.5.5 → 2.5.6.

> 2026-09-03 MLB 경기센터 업데이트: 메인 창을 1180×820 데스크톱 레이아웃으로 확장하고 MLB 상세 화면에 이닝 스코어, 팀 비교 통계, 경기 리더, 타자·투수 박스스코어, 부상자 명단, 현재 승부와 전체 플레이를 한 화면에 통합했다. 버전 2.5.6 → 2.6.0.
...
결과물은 `orange-sports/final-release/`에 생성된다. `node_modules`, `dist`, `release`, `final-release`는 Git에서 제외했으므로 집에서는 반드시 `npm install` 후 다시 빌드해야 한다.
...
브라우저에서 `http://127.0.0.1:4173`을 연다. 별도 패키지 의존성이 없으므로 현재는 `npm install` 없이 실행할 수 있다. 종료는 실행한 터미널에서 `Ctrl+C`를 누른다.

## 2026-09-03 작업 종료 메모

- `orange-sports-web/`에 스포츠 통합 홈과 MLB·NBA·EPL·UCL·F1 종목 화면을 만들었다.
- 날짜별 경기·결과, 상단 스코어 띠, 종목별 순위, 뉴스 카드, 관심 팀 로컬 저장과 반응형 UI가 포함되어 있다.
- 기사 클릭 시 외부 페이지로 즉시 이동하지 않고 내부 리더 화면을 표시하며, 원문 버튼은 별도로 둔다. 기사 전문은 수집하지 않는다.
- 뉴스 썸네일은 검색 결과가 제공한 이미지를 사용하고, 이미지가 없으면 종목별 대체 그래픽을 표시한다.
- 국내 뉴스는 네이버 검색 HTML을 읽고 실패하면 Google News RSS를 사용한다. 이는 개인용 실험 구현이며 공개 서비스용 수집 방식으로 간주하지 않는다.
- 공개 사이트·광고·유료화는 중단했다. 집에서는 경기 상세 및 문자중계를 로컬 웹 화면에 이식하고 Windows 알림과 연결하는 작업부터 이어간다.
- 이번 정리에서는 새 설치 파일을 만들지 않았다. Windows 앱 최신 소스 버전은 2.6.0이며 필요할 때 집에서 `npm install` 후 `npm run build`로 패키징한다.

## 현재 구현 범위

### 통합관리자 웹 스포츠 위젯 — 사용 종료

아래 항목은 이전 구현 기록이다. 통합관리자 페이지에서는 더 이상 로드하거나 실행하지 않는다. 원본 JS는 `legacy-admin-widget/widget-sports.js`로 옮겼고 `.sp-*` 스타일은 Orange Sports 웹 전환 시 참고할 레거시 소스로 당분간 보존한다.
...
- `index.html`
- `css/panels.css`
- `legacy-admin-widget/widget-sports.js`

### Windows 스포츠 라이브 위젯

`orange-sports/`에 Electron 기반 독립 앱을 만들었다. 설치 제품명은 `Orange Sports`이며 현재 버전은 2.6.0이다.

- 프레임 없는 1180×820 데스크톱 창(최소 720×600)
...
- 구간 종료 알림은 NBA 각 쿼터·연장 종료, MLB 매 이닝 초·말 종료, EPL/UCL 전반전·경기 종료에 발생하며 종료 시점의 현재 점수를 본문과 스코어 영역에 함께 표시한다.
...
- `orange-sports/main.js`: 창, 트레이, 설정 저장, 시작 프로그램, 카드 알림 창, 스포츠 데이터 프록시(허용 호스트 목록)
- `orange-sports/preload.js`: 안전한 IPC 공개
- `orange-sports/renderer.js`: 경기 목록, 문자중계, 폴링, 알림 판정, **순위 모듈**(파일 하단 `TEAM_KO_MAP`·`getKoName`·`tblStandings`·`tbl*StatLeaders`·`tbl*Playoffs`·`tblUclTournament`·`loadTable`·`renderTable`·`onTableClick`)
- `orange-sports/index.html`, `styles.css`: UI(순위 탭·`#tableView`·`.sp-*` 이식 스타일 포함)
- `orange-sports/notification.html`, `notification.css`, `notification.js`: 카카오톡형 적층 알림 UI
- `orange-sports/notification-preload.js`: 알림 창 전용 IPC
- `orange-sports/package.json`: 실행/검사/NSIS 빌드
...
웹 위젯에는 MLB Stats API와 ESPN의 축구·농구 데이터 주소도 사용했다. 당시 전체 상수와 요청 코드는 `legacy-admin-widget/widget-sports.js`에서 확인한다.
...
- NBA 종료 플레이를 일반 플레이와 분리해 각 쿼터·연장·경기 종료 알림을 놓치지 않고 현재 점수를 함께 표시한다.
- MLB `inningState` 및 초·말 전환을 추적해 `1회 초 종료`부터 매 이닝 초·말 종료 알림을 추가했다.
- EPL/UCL은 ESPN 경기 상태의 Halftime/Full Time과 전·후반 전환을 추적해 전반전·경기 종료 알림을 추가했다.
- 경기 알림을 처음 켰을 때는 현재 구간을 기준점으로만 저장하여 이미 지난 종료 이벤트를 소급 알림하지 않는다.
- 상태별 종료 키를 저장해 같은 휴식 구간을 12초마다 반복 알림하지 않는다.
- `npm run check` 및 전체 `npm run smoke` 통과. 상태 전환 검사 결과 MLB `1-top`·`1-bottom`, 축구 `halftime`·`fulltime`을 확인했고 기존 종목 상세와 F1 테스트도 통과했다.
- `Orange-Sports-Setup-2.5.1.exe` 패키징 완료, 크기 93,786,917 bytes(약 89.44 MiB), SHA-256 `663EA157C897CDAA26F676D0E7A8D9914B9569FE1F5D413257793A7F7CE5A059`.
- 빌드는 `release-2.5.1`에서 완료한 뒤 설치 파일과 blockmap을 `final-release`에 복사했다. 코드 서명 인증서가 없어 Authenticode는 `NotSigned`다.

### 2.5.2 (2026-09-03)

- 개별 경기 알림 선택 목록이 비어 있을 때만 응원 팀 필터 또는 전체 경기 설정을 사용한다.
- 개별 경기를 하나 이상 선택하면 해당 경기 ID만 감시한다. 응원 팀이 비어 있어도 다른 진행 중 경기를 함께 알리지 않는다.
- 실제 사용자 설정에서 MLB 경기 `823339` 하나가 선택되고 응원 팀은 비어 있는 상태를 확인해 문제를 재현했다.
- `npm run check`와 패키징 통과. `Orange-Sports-Setup-2.5.2.exe` 크기 93,786,974 bytes(약 89.44 MiB), SHA-256 `C14E03E739BCB562BAFB77C78419C8F30607A798443341181FB5A6A81FF585BF`, Authenticode `NotSigned`.

### 2.5.3 (2026-09-03)

- `orange-sports/icon.png`을 메인 창, 카드 알림 창, 시스템 트레이 아이콘에 적용했다.
- electron-builder Windows 아이콘에도 같은 파일을 지정해 실행 파일, 바탕화면 바로가기, 시작 메뉴 및 설치 프로그램에 반영한다.
- `npm run check` 및 Windows NSIS 패키징 통과. 빌드된 `app.asar`에 `icon.png`이 포함된 것도 확인했다.
- `Orange-Sports-Setup-2.5.3.exe` 크기 95,619,315 bytes(약 91.19 MiB), SHA-256 `591646CA0778ABE1BD71451264D3E10364A389B36059D37063E54755378782B4`, Authenticode `NotSigned`.

### 2.5.4 (2026-09-03)

- 알림 창의 외부 배치 여백 8px과 내부 투명 여백 12~14px가 합산되던 구조를 수정했다.
- 카드 콘텐츠 크기는 유지하면서 창 크기를 390×132, 투명 여백을 3px, 화면 배치 여백을 0px로 조정해 실제 카드가 작업영역 우측·하단에서 약 3px만 떨어진다.
- 여러 알림이 쌓일 때도 오른쪽 기준선을 유지하고 카드 사이에는 약 6px 간격을 둔다.
- 화면 가장자리에서 잘리지 않도록 그림자를 짧고 부드러운 `0 1px 4px` 형태로 조정했다.
- `npm run check` 및 Windows NSIS 패키징 통과. `Orange-Sports-Setup-2.5.4.exe` 크기 95,619,265 bytes(약 91.19 MiB), SHA-256 `B5AFF23DD0A1EC2BAAECB9CF18FAE597BE14CDF4837495A9524F1979E4414920`, Authenticode `NotSigned`.

### 2.5.5 (2026-09-03)

- 사용자가 교체한 224×224 `icon.png`을 앱 창과 트레이에 반영한다. electron-builder의 최소 256×256 제한 때문에 같은 이미지를 512×512로 확대한 `icon-build.png`을 실행 파일 및 설치 프로그램용으로 사용한다.
- 알림 카드의 모서리 반경을 17px에서 11px로 줄여 더 단정한 형태로 조정했다.
- `npm run check` 및 Windows NSIS 패키징 통과. `Orange-Sports-Setup-2.5.5.exe` 크기 94,027,030 bytes(약 89.67 MiB), SHA-256 `2D75ADA4FD897093FDA88E3304413EE186D19FD67FECC68D923A59E430865D62`, Authenticode `NotSigned`.

### 2.5.6 (2026-09-03)

- 응원 팀 경기와 직접 찜한 경기를 우선순위가 아닌 합집합으로 감시한다. 서로 다른 두 경기가 동시에 진행되면 두 경기 알림이 모두 오며, 동일 경기가 양쪽 조건에 걸려도 한 번만 처리한다.
- 화면에 열어둔 종목의 `state.events`만 보던 구조를 제거하고, 설정이 있는 NBA·MLB·EPL·UCL의 당일 라이브 일정을 각각 백그라운드에서 조회한다.
- 야구 화면을 보고 있어도 찜한 NBA 경기가 진행 중이면 NBA 알림이 함께 오며, 반대 경우와 EPL/UCL도 동일하다.
- 한국 시간 날짜 경계 누락을 막기 위해 ESPN 종목은 전일~당일 범위를 조회한 뒤 `Asia/Seoul` 날짜와 라이브 상태를 다시 필터링한다. MLB도 전일~당일 일정에서 한국 시간 당일 경기만 사용한다.
- NBA 응원 팀 약어 `GSW/GS`, `NOP/NO`, `NYK/NY`, `SAS/SA`, `UTA/UTAH`, `WAS/WSH` 차이를 정규화했다.
- 실제 설정의 MLB 응원 팀 `119`(LA 다저스), 종료된 선택 경기 `823339`, 현재 라이브 경기 `823906` 조합으로 원인을 확인했다.
- `npm run check` 및 Windows NSIS 패키징 통과. `Orange-Sports-Setup-2.5.6.exe` 크기 94,027,402 bytes(약 89.67 MiB), SHA-256 `FF257D7CCB2B4544B021486039B15B097ACDF230A193AB60065BF17E05B83DAE`, Authenticode `NotSigned`.

### 2.6.0 (2026-09-03)

- 메인 창 기본 크기를 1180×820, 최소 크기를 720×600으로 확장했다. 알림 카드는 기존 소형 크기를 그대로 유지한다.
- MLB 경기 상세를 데스크톱 경기센터 구조로 재배치했다: 경기 헤더, 이닝별 R/H/E, 팀 비교 통계, 경기 리더, 양 팀 타자·투수 박스스코어, 부상 보고서, 현재 타석, 득점 장면, 전체 플레이 기록.
- 선수 박스스코어는 현재 경기 수치와 시즌 AVG·OBP·ERA를 함께 표시한다. 경기 리더와 부상 보고서에는 MLB 선수 헤드샷과 팀 로고를 사용한다.
- 부상자 명단은 MLB 40인 로스터의 D10/D15/D60 상태를 조회하며 10분 캐시한다. 부상 API가 실패해도 경기 상세 본문은 계속 렌더링한다.
- 1040px 이하에서는 박스스코어와 문자중계를 한 열로 전환하는 반응형 구조를 유지한다.
- `npm run check` 및 Windows NSIS 패키징 통과. `Orange-Sports-Setup-2.6.0.exe` 크기 94,030,328 bytes(약 89.67 MiB), SHA-256 `9A3CA020E71E322B43240EF1C699381E00DEDCAEA3374D33283EBEDF4B3FCC0D`, Authenticode `NotSigned`.
...
- 2.1.0(알림 개선 + 순위 뷰 이식)은 현재 `orange-sports/` 하위 소스 파일과 이 문서, `README.md` 변경으로 구성된다.
