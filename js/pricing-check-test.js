...
(() => {
  ...
  window.openPriceCheckTest=function(tabId) {
    ...
    dialog.innerHTML=`<div class="pricing-input-modal-header">
      ...
      <div class="pv-body">
        <div class="pv-hint"><i class="fa-solid fa-circle-info"></i>
          <span>선택한 카테고리의 등록 상품을 실제 적용 단가 기준으로 비교합니다. 상품 탭에서 수집한 할인·옵션 가격을 쓰며, 직접 API 조회는 사용하지 않습니다.<br>통합 확장 0.29.13 이상을 설치한 Chrome에서 실행하세요.</span>
        </div>
    ...
    dialog.querySelector('[data-run]').onclick=async()=>{
      ...
      try {
        status.textContent='확장 연결 확인 중…';const extension=await request('ping');if(!extension.version || compareExtensionVersions(extension.version,'0.29.13')<0)throw Error('통합 확장을 0.29.13 이상으로 업데이트·리로드해주세요.');
