...
function _costCard(tabId, modalType, titleSub, tableBodyHtml, hiddenHtml) {
  ...
  return `<div class="card pricing-cost-card">
    ...
    <div class="pricing-cost-footer">
      ...
      <button type="button" class="pricing-margin-edit-btn" onclick="openPriceCheckTest('${tabId}')" title="지금 보고 있는 카테고리 상품을 실제 적용 단가와 비교합니다(다른 카테고리도 창에서 바로 바꿀 수 있음)">스토어 가격검사</button>
      <button type="button" class="pricing-margin-edit-btn" onclick="openCompetitorPriceCheck('${tabId}')" title="이 탭에 등록해둔 경쟁사 가격/링크가 실제 스마트스토어 가격과 같은지 확인합니다(스마트스토어 링크만 지원)">
        <i class="fa-solid fa-people-arrows"></i> 경쟁사 가격 확인
      </button>
