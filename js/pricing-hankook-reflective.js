/* ═══════════════════════════════════════
   ...
   - 롤형(5T/6T/10T/13T + 고티 20~50T): 원가를 원재료(PE폼·PET필름 등) 단가로 더 쪼개는 공식은 못
     찾았다 — 에너가드론(다른 브랜드) 파일의 재료비 공식을 가져와 봐도 5T~13T는 두께별 비율이 전혀
     안 맞았고, 고티(20T~50T)는 판상형과 같은 제품인데도(사용자 확인 — "20T를 롤로도 팔고 2M로 잘라
     판상형으로도 판다") 판상형 재료비 공식과 정확히 일치하진 않았다(1~10% 오차 — 판상형 쪽 재단·
     펼치기 가공비가 얹힌 걸로 보임, 단열벽지 2.3M과 같은 이유). 그런데 **"두께·등급별 마스터 원가
     (=롤 전체 원가) × 판매 길이 비례"는 정상적으로 계산된다**(HK_REFLECTIVE_ROLL_MASTER,
     _hkReflectiveRollCost — 단열벽지 롤 단위와 같은 방식). 2026-09-22엔 "원재료로 더 못 쪼갠다"는
     이유로 원가 칸 자체를 숨겼었는데, 2026-09-23에 단열벽지와 같은 기준(길이 비례 계산도 "계산되는
     것"으로 침)으로 다시 보여주기로 했다 — 편집 카드(_hkReflectiveRollCostCard)도 다시 켰다.
   ...
   구조는 여기(JS), 값은 DB — 저장하는 값은 판매가(상품코드별)와 원가 설정(hk_settings.reflective_costs,
   { bom: 판상형 재료 구성비, roll: 롤형 마스터 원가 })뿐이다. 마진·수수료·순수마진은 화면에서
   다시 계산한다(부자재와 같은 방식).
═══════════════════════════════════════ */
...
function _hkReflectiveTableHtml(items) {
  // 롤형·판상형 둘 다 원가를 보여준다(사용자 확인 2026-09-23 — 단열벽지처럼 "마스터 원가×길이"로
  // 계산되는 건 원재료 단가로 더 쪼개지지 않아도 "계산되는 것"으로 친다). 롤형은 HK_REFLECTIVE_ROLL_MASTER
  // 마스터 원가를 길이 비례로 계산(_hkReflectiveRollCost), 판상형은 BOM 카드로 계산.
  const showCost = true;
...
function renderHkReflectivePane() {
  // 롤형 원가 칸(표)은 다시 보여주지만(마스터 원가×길이 계산, 단열벽지와 같은 기준) 편집 카드는
  // 안 켠다(사용자 확인 2026-09-23 — 단열벽지처럼 원가표 없이 그냥 계산된 값만 보여주는 쪽으로).
  // _hkReflectiveRollCostCard()는 안 쓰지만 남겨둔다.
